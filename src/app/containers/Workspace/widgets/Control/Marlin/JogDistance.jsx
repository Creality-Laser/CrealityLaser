...
class JogDistance extends PureComponent {
  ...
  actions = {
    ...
    increaseCustomDistance: () => {
      const { customValue, step, max } = this.state;

      const value = Math.min(customValue + step, max);
    ...
    decreaseCustomDistance: () => {
      const { customValue, step, min } = this.state;

      const value = Math.max(customValue - step, min);
