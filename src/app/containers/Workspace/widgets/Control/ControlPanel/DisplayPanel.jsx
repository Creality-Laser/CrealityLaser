    ...
    render() {
        ...
        return (
            <div className={styles['display-panel-wrapper']}>
                <table className={classNames(styles.displayPanelTable, !canClick ? styles.displayPanelTableDisabled : "")}>
                    <thead>
                        <tr>
                            <th className="nowrap" title={i18n._('Axis')}>{i18n._('Axis')}</th>
                            <th title={i18n._('Machine Coordinates')}>{i18n._('Machine Coordinates')}</th>
                            <th title={i18n._('Work Coordinates')}>{i18n._('Work Coordinates')}</th>
...
class DropdownButton extends PureComponent {
    ...
    return (
        ...
        >
            <div
                className={classNames(styles.dropdownHeader, disabled ? styles.dropdownHeaderDisabled : "")}
                onClick={() => {
                    if (disabled) {
                        return;
                    }
            ...
            >
                ...
                <span><i className={classNames("iconfont", styles.dropdownIcon, isDropped ? styles.dropdownIconUp : "")}>&#xe6bd;</i></span>
            </div>
            {isDropped && (
            <div className={styles.dropdownPanelWrapper}>{items.map(({ label: innerLabel, key, onClick, disabled: innerDisabled }) => {
        return (
            <div
                className={classNames(styles.dropdownItem, innerDisabled ? styles.dropdownItemDisabled : "")}
                key={key}
                onClick={() => {
                if (innerDisabled) {
                    return;
