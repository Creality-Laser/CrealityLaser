...
class Console extends PureComponent {
  ...
  actions = {
    ...
    queryGCommands: () => {
      ...
      if (terminal) {
        ...
        terminal.writeln(
          color.green('  G4: Pause the Machine for Seconds or Milliseconds')
        );
    ...
    queryMCommands: () => {
      ...
      if (terminal) {
        ...
        terminal.writeln(
          color.yellow('  M109: Set Extruder Temperature and Wait')
        );
        ...
        terminal.writeln(
          color.yellow('  M220: Set Speed Factor override Percentage')
        );
        terminal.writeln(
          color.yellow('  M221: Set Extruder Factor override Percentage')
        );
        ...
        terminal.writeln(
          color.yellow('  M421: Set a Mesh Bed Leveling Z coordinate')
        );
    ...
    greetings: () => {
      ...
      if (this.props.isConnected && this.props.port) {
        ...
        if (terminal) {
          terminal.writeln(`${name} ${version}`);
          terminal.writeln(
            i18n._('Connected to {{-port}}', { port: this.props.port })
          );
        }
      }
    ...
    if (
      ...
      if (terminal) {
        if (nextProps.connectionType === CONNECTION_TYPE_SERIAL) {
          terminal.writeln(`${name} ${version}`);
          terminal.writeln(
            i18n._('Connected to {{-port}}', { port: nextProps.port })
          );
        } else {
          terminal.writeln(`${name} ${version}`);
          terminal.writeln(i18n._('Connected via Wi-Fi'));
  ...
  render() {
    const { isDefault, terminalHistory, history, isMaximized } = this.props;
