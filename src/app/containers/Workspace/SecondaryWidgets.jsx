...
class SecondaryWidgets extends PureComponent {
  ...
  onChangeWidgetOrder = (widgets) => {
    this.props.updateTabContainer('secondary', { widgets });
  };

  render() {
    const { className = '', defaultWidgets } = this.props;

    const widgets = this.props.secondaryWidgets.map((widgetId) => (
      <div
        data-widget-id={widgetId}
        key={widgetId}
        style={{
          display: _.includes(defaultWidgets, widgetId) ? 'none' : 'block',
        }}
      >
        <Widget
          widgetId={widgetId}
