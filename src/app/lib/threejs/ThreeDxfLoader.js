...
class ThreeDxfLoader {
  ...
  drawCircle(geometry, entity, data) {
    ...
    const interpolatedPoints = new THREE.CircleGeometry(
      entity.radius,
      32,
      entity.startAngle,
      entity.angleLength
    ...
    if (
      !isEqual(previousPosition.x, interpolatedPoints.vertices[0].x) ||
    ...
    geometry.colors.push(
      ...new Array(interpolatedPoints.vertices.length).fill(color)
    );
  }
  ...
  drawEllipse(geometry, entity, data) {
    ...
    if (
      !isEqual(previousPosition.x, interpolatedPoints[0].x) ||
  ...
  drawSpline(splineGeo, entity, data) {
        ...
        if (entity.controlPoints.length === 4) {
          ...
          for (let t = 0; t < 1; t += 0.1) {
            p0 =
              ...
              3 * t ** 2 * (1 - t) * entity.controlPoints[2].x +
              t ** 3 * entity.controlPoints[3].x;
            p1 =
              (1 - t) ** 3 * entity.controlPoints[0].y +
              3 * t * (1 - t) ** 2 * entity.controlPoints[1].y +
              3 * t ** 2 * (1 - t) * entity.controlPoints[2].y +
              t ** 3 * entity.controlPoints[3].y;
    ...
    if (
      !isEqual(previewPositon.x, interpolatedPoints[0].x) ||
  ...
  normalizer(dxfString, scale) {
    ...
    for (const entities of dxf.entities) {
      if (
        ...
        center.x = this.translateAndScale(center.x, centerX, scale);
        center.y = this.translateAndScale(center.y, centerY, scale);
        majorAxisEndPoint.x = this.translateAndScale(
          majorAxisEndPoint.x,
          0,
          scale
        );
        majorAxisEndPoint.y = this.translateAndScale(
          majorAxisEndPoint.y,
          0,
          scale
        );
