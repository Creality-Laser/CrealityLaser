...
import { cleanupAttributes, setAttributes, getBBox } from './element-utils';
import {
  transformPoint,
  getTransformList,
  getRotationAngle,
...
class SVGCanvas extends PureComponent {
  ...
  onMouseDown = (event) => {
    ...
    this.currentDrawing = { ...CURRENTDRAWING_INIT };
    ...
    switch (this.mode) {
      ...
      case 'resize': {
        ...
        if (hasMatrix) {
          ...
          transformList.insertItemBefore(
            this.svgContainer.createSVGTransform(),
            pos
          );
          transformList.insertItemBefore(
            this.svgContainer.createSVGTransform(),
  ...
  onMouseMove = (event) => {
    ...
    const x = pt.x;
    const y = pt.y;
    const element = this.svgContentGroup.findSVGElement(
      this.svgContentGroup.getId()
    );
      ...
      case 'fhpath': {
        ...
        if (draw.controlPointX2 && draw.controlPointY2) {
          ...
          for (let i = 0; i < STEP_COUNT - 1; i++) {
            ...
            draw.sumDistance += Math.sqrt(
              (nextX - curX) * (nextX - curX) + (nextY - curY) * (nextY - curY)
            );
  ...
  onMouseUp = (event) => {
    ...
    draw.started = false;
    let element = this.svgContentGroup.findSVGElement(
      this.svgContentGroup.getId()
    );
    const matrix = this.svgContentGroup.getScreenCTM().inverse();
    const pt = transformPoint({ x: event.pageX, y: event.pageY }, matrix);
  ...
  getBsplinePoint(t) {
    ...
    spline.x =
      S *
      ((p0.x * m[0][0] + p1.x * m[0][1] + p2.x * m[0][2] + p3.x * m[0][3]) *
        t3 +
        (p0.x * m[1][0] + p1.x * m[1][1] + p2.x * m[1][2] + p3.x * m[1][3]) *
          t2 +
        (p0.x * m[2][0] + p1.x * m[2][1] + p2.x * m[2][2] + p3.x * m[2][3]) *
          t +
        (p0.x * m[3][0] + p1.x * m[3][1] + p2.x * m[3][2] + p3.x * m[3][3]));

    spline.y =
      S *
      ((p0.y * m[0][0] + p1.y * m[0][1] + p2.y * m[0][2] + p3.y * m[0][3]) *
        t3 +
        (p0.y * m[1][0] + p1.y * m[1][1] + p2.y * m[1][2] + p3.y * m[1][3]) *
          t2 +
        (p0.y * m[2][0] + p1.y * m[2][1] + p2.y * m[2][2] + p3.y * m[2][3]) *
          t +
  ...
  updateCanvas = (size) => {
    ...
    this.printableArea.updateScale({ size, scale: this.scale });
  ...
  render() {
    ...
    return <div ref={this.node} className={className} />;
  }
}
