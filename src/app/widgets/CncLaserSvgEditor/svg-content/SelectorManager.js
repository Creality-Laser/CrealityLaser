...
class Selector {
  ...
  showGrips(show) {
    this.manager.selectorGripsGroup.setAttribute(
      'display',
      show ? 'inline' : 'none'
    );
  ...
  resize(bbox) {
    ...
    if (angle) {
      ...
      const minx =
        Math.min(
          nbox.tl.x,
          Math.min(nbox.tr.x, Math.min(nbox.bl.x, nbox.br.x))
        ) - offset;
      const miny =
        Math.min(
          nbox.tl.y,
          Math.min(nbox.tr.y, Math.min(nbox.bl.y, nbox.br.y))
        ) - offset;
      const maxx =
        Math.max(
          nbox.tl.x,
          Math.max(nbox.tr.x, Math.max(nbox.bl.x, nbox.br.x))
        ) + offset;
      const maxy =
        Math.max(
          nbox.tl.y,
          Math.max(nbox.tr.y, Math.max(nbox.bl.y, nbox.br.y))
        ) + offset;
    ...
    this.manager.rotateGripConnector.setAttribute(
      'y2',
      ny - (GRIP_RADIUS * 9.4) / this.manager.scale
    );
    this.manager.rotateGrip.setAttribute('cx', nx + nw / 2);
    this.manager.rotateGrip.setAttribute(
      'cy',
      ny - (GRIP_RADIUS * 9.4) / this.manager.scale
    );
  }
...
class SelectorManager {
  ...
  initGroup() {
    ...
    this.rotateGrip = createSVGElement({
      ...
      attr: {
        ...
        r: GRIP_RADIUS / this.scale,
        'stroke-width': 2 / this.scale,
        style: 'cursor:url(../assets/rotate.png) 12 12, auto;',
      },
    });
  ...
  updateScale(scale) {
    ...
    this.rotateGripConnector.setAttribute(
      'y2',
      ny - (GRIP_RADIUS * 9.4) / this.scale
    );
