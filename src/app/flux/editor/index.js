...
import {
  ...
  computeTransformationSizeForTextVector,
} from './base';
import {
  SVG_EVENT_ADD,
  SVG_EVENT_MOVE,
...
import { PAGE_EDITOR, PAGE_PROCESS, DATA_PREFIX } from '../../constants';
...
export const actions = {
  ...
  uploadCaseImage:
    (headType, file, mode, caseConfigs, caseTransformation, onError) =>
    (dispatch) => {
      api
        .uploadLaserCaseImage(file)
        .then((res) => {
          const { width, height, originalName, uploadName } = res.body;
  ...
  generateModel:
    ...
    (dispatch, getState) => {
      ...
      let { width, height } = sizeModelByMachineSize(
        size,
        sourceWidth,
        sourceHeight
      );
      if (sourceType === 'text') {
        const textSize = computeTransformationSizeForTextVector(
          DEFAULT_TEXT_CONFIG.text,
          DEFAULT_TEXT_CONFIG.size,
          DEFAULT_TEXT_CONFIG.lineHeight,
          {
            width: sourceWidth,
            height: sourceHeight,
          }
      ...
      if (!checkParams(headType, sourceType, mode)) {
        console.error(
          `sourceType or mode error, sourceType:${sourceType}, mode:${mode}`
        );
        return;
      }

      const defaultConfig = modelDefaultConfigs.config;
      const defaultGcodeConfig =
        headType === 'cnc'
          ? {
      ...
      const defaultTransformation =
        `${headType}-${sourceType}-${mode}` === 'cnc-raster-greyscale'
          ? {
              width: 40,
  ...
  genQrcodeModel: (headType, obj) => async (dispatch) => {
    const { originalName, uploadName, width, height } = obj;
  ...
  genQrcode: (headType, text) => async () => {
    try {
      const res = await api.genQrcode({
        text,
      });
  ...
  changeSelectedModelMode:
    (headType, sourceType, mode) => async (dispatch, getState) => {
      ...
      api.processImage(options).then((res) => {
        let processImageName = res.body.filename;

        // if preview image, use it prefer.
  ...
  removeSelectedModel: (headType) => (dispatch, getState) => {
    ...
    try {
      ...
      const toolPathModelState =
        toolPathModelGroup.removeSelectedToolPathModel();
      dispatch(
        baseActions.updateState(headType, {
          ...modelState,
          ...toolPathModelState,
  ...
  onSetSelectedModelPosition: (headType, position) => (dispatch, getState) => {
    ...
    dispatch(
      actions.updateSelectedModelTransformation(headType, transformation)
    );
  ...
  onModelTransform: (headType) => (dispatch, getState) => {
    ...
    toolPathModelGroup.updateSelectedNeedPreview(true);
    dispatch(
      actions.changeModelVisualizer(headType, modelState.modelID, false)
    );
  ...
  updateSelectedModelTextConfig:
    (headType, newConfig) => (dispatch, getState) => {
      const { modelGroup, svgModelGroup, toolPathModelGroup, config } =
        getState()[headType];
      newConfig = {
        ...config,
        ...newConfig,
      };
      api.convertTextToSvg(newConfig).then(async (res) => {
        const { originalName, uploadName, width, height } = res.body;
        ...
        const transformation = {
          width:
            (selectedModel.transformation.width / selectedModel.width) *
            textSize.width,
          height:
            (selectedModel.transformation.height / selectedModel.height) *
            textSize.height,
        };
        ...
        dispatch(
          baseActions.updateTransformation(headType, {
            ...textSize,
          })
        );
        dispatch(baseActions.resetCalculatedState(headType));
  ...
  previewModel: (headType, isProcess) => (dispatch, getState) => {
    ...
    if (isProcess || autoPreviewEnabled) {
      ...
      if (modelState) {
        const toolPathModelTaskInfo =
          toolPathModelGroup.getToolPathModelTaskInfo(modelState.modelID);
        if (
          toolPathModelTaskInfo &&
          toolPathModelTaskInfo.needPreview &&
          !toolPathModelTaskInfo.hideFlag
        ) {
          const taskInfo = {
            ...modelState,
            ...toolPathModelTaskInfo,
  ...
  manualPreview: (headType, isProcess) => (dispatch, getState) => {
    ...
    if (isProcess || autoPreviewEnabled) {
      for (const model of modelGroup.getModels()) {
        ...
        const toolPathModelTaskInfo =
          toolPathModelGroup.getToolPathModelTaskInfo(modelTaskInfo.modelID);
        if (
          toolPathModelTaskInfo &&
          toolPathModelTaskInfo.needPreview &&
          !toolPathModelTaskInfo.hideFlag
  ...
  changeModelVisualizer:
    (headType, modelID, isProcess) => (dispatch, getState) => {
  ...
  initModelsPreviewChecker: (headType) => (dispatch, getState) => {
    const { modelGroup, toolPathModelGroup, isAllModelsPreviewed } =
      getState()[headType];
    const check = () => {
      const isAllModelsPreviewedN = checkIsAllModelsPreviewed(
        modelGroup,
        toolPathModelGroup
      );
      if (isAllModelsPreviewedN !== isAllModelsPreviewed) {
        dispatch(
          baseActions.updateState(headType, {
            isAllModelsPreviewed: isAllModelsPreviewedN,
          })
        );
      }
  ...
  onReceiveTaskResult: (headType, taskResult) => async (dispatch, getState) => {
    ...
    if (
      ...
      toolPathModelGroup.getToolPathModelByID(data.id)
    ) {
      dispatch(
        baseActions.updateState(headType, {
          previewFailed: true,
    ...
    const toolPathModelState = await toolPathModelGroup.receiveTaskResult(
      data,
      filename
    );
    ...
    if (toolPathModelState) {
      dispatch(
        actions.changeModelVisualizer(
          headType,
          toolPathModelState.modelID,
          true
        )
      );
      dispatch(actions.togglePage(headType, PAGE_PROCESS));
  ...
  updateSelectedModelPrintOrder:
    (headType, printOrder) => (dispatch, getState) => {
      const { toolPathModelGroup } = getState()[headType];
  ...
  updateSelectedModelGcodeConfig:
    (headType, gcodeConfig) => (dispatch, getState) => {
      const { toolPathModelGroup } = getState()[headType];
  ...
  toggleToolPathVisible: (headType) => (dispatch, getState) => {
    const { modelGroup, toolPathModelGroup } = getState()[headType];
  ...
  togglePage: (headType, page) => (dispatch, getState) => {
    ...
    if (page === PAGE_EDITOR) {
      ...
      dispatch(actions.manualPreview(headType, true));
    }
    dispatch(baseActions.render(headType));
  },
  ...
  generateGcode: (headType, thumbnail) => (dispatch, getState) => {
    ...
    for (const model of modelGroup.getModels()) {
      ...
      const modelTaskInfo = model.getTaskInfo();
      const toolPathModelTaskInfo = toolPathModelGroup.getToolPathModelTaskInfo(
        modelTaskInfo.modelID
      );
    ...
    const orderModelInfos = modelInfos
      .map((d) => d)
      .sort((d1, d2) => {
  ...
  undo: (headType) => (dispatch, getState) => {
    const { modelGroup, toolPathModelGroup, undoSnapshots, redoSnapshots } =
      getState()[headType];
    ...
    const toolPathModelState = toolPathModelGroup.undoRedo(
  ...
  hideSelectedModel: (headType) => (dispatch, getState) => {
    try {
      ...
      console.log('hideSelectedModel catch', error);
    }
  },
  ...
  showSelectedModel: (headType) => (dispatch, getState) => {
    ...
    svgModelGroup.updateTransformation(
      modelGroup.getSelectedModel().transformation
    );
    dispatch(baseActions.render(headType));
  },
};
