...
class SvgModelGroup {
  ...
  svgToTransformation(elem) {
    const { width, height, positionX, positionY, rotationZ } =
      coordGmSvgToModel(this.size, elem);
  ...
  svgToModel(elem) {
    const { x, y, width, height, positionX, positionY } = coordGmSvgToModel(
      this.size,
      elem
    );
    ...
    const content =
      `<svg x="0" y="0" width="${width * DEFAULT_SCALE}" height="${
        height * DEFAULT_SCALE
      }" ` +
      `viewBox="${x * DEFAULT_SCALE} ${y * DEFAULT_SCALE} ${
        width * DEFAULT_SCALE
      } ${height * DEFAULT_SCALE}" ` +
  ...
  addImageBackgroundToSVG(model) {
    const { x, y, width, height } = coordGmModelToSvg(
      this.size,
  ...
  selectElementById(modelID) {
    if (this.svgContentGroup.findSVGElement) {
  ...
  updateTransformation(transformation, elem) {
    ...
    if (
      transformation.positionX !== undefined ||
      transformation.positionY !== undefined
    ) {
    ...
    if (
      (transformation.width !== undefined ||
        transformation.height !== undefined) &&
      !isZero(bbox.width) &&
      !isZero(bbox.height)
    ) {
