...
class ModelGroup {
  ...
  addModel(model) {
    if (model) {
      ...
      if (
        model.sourceType === '3d' &&
        model.transformation.positionX === 0 &&
        model.transformation.positionZ === 0
      ) {
        model.stickToPlate();
  ...
  getSortedModelsByPositionZ() {
    ...
    for (let i = 0; i < length; i++) {
      for (let j = 0; j < length - i - 1; j++) {
        if (
          sorted[j].meshObject.position.z > sorted[j + 1].meshObject.position.z
        ) {
          const tmp = sorted[j];
          sorted[j] = sorted[j + 1];
          sorted[j + 1] = tmp;
  ...
  arrangeAllModels() {
    ...
    return this.selectedModel
      ? this.getState(this.selectedModel)
      : this._emptyState;
  }
  ...
  _computeAvailableXY(model) {
    ...
    for (let stepCount = 1; stepCount < length / step; stepCount++) {
      ...
      for (const position of checkPositions) {
        ...
        if (
          modelBox3Clone.min.x < this._bbox.min.x ||
  ...
  getAllBoundingBox() {
    ...
    for (const model of this.models) {
      ...
      boundingBox.max.x = boundingBox.max.x
        ? Math.max(boundingBox.max.x, modelBoundingBox.max.x)
        : modelBoundingBox.max.x;
      boundingBox.max.y = boundingBox.max.y
        ? Math.max(boundingBox.max.y, modelBoundingBox.max.y)
        : modelBoundingBox.max.y;
      boundingBox.max.z = boundingBox.max.z
        ? Math.max(boundingBox.max.z, modelBoundingBox.max.z)
        : modelBoundingBox.max.z;
      boundingBox.min.x = boundingBox.min.x
        ? Math.min(boundingBox.min.x, modelBoundingBox.min.x)
        : modelBoundingBox.min.x;
      boundingBox.min.y = boundingBox.min.y
        ? Math.min(boundingBox.min.y, modelBoundingBox.min.y)
        : modelBoundingBox.min.y;
      boundingBox.min.z = boundingBox.min.z
        ? Math.min(boundingBox.min.z, modelBoundingBox.min.z)
