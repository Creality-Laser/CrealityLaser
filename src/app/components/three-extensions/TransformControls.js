    ...
    if (mode === 'translate') {
      ...
      if (axis.search('XYZ') !== -1) {
        ...
        if (scope.uniformScale) {
          ...
          // set limit: the object size must be larger than (10, 10, 10);
          scaleFactor =
            scaleFactor < uniformScaleMinValue
              ? uniformScaleMinValue
              : scaleFactor;
      ...
      const quaternion =
        this.space === 'local' ? worldQuaternion : _identityQuaternion;
      ...
      if (axis === 'E') {
        ...
        rotationAngle =
          pointEnd.angleTo(pointStart) * (_tempVector.dot(eye) < 0 ? 1 : -1);
      } else if (axis === 'XYZE') {
        _tempVector.copy(pointEnd).sub(pointStart).cross(eye).normalize();
        rotationAxis.copy(_tempVector);
        rotationAngle =
          pointEnd.sub(pointStart).dot(_tempVector.cross(eye)) * ROTATION_SPEED;
        ...
        rotationAngle =
          _tempVector2.dot(_tempVector.cross(eye).normalize()) * ROTATION_SPEED;
      }
...
TransformControls.prototype = Object.assign(
