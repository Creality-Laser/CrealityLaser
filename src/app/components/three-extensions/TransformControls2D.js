...
import ico from './rotate_16x16.ico';
...
THREE.TransformControls2D = function (camera, domElement) {
  ...
  function onMouseDown(event) {
    ...
    if (event.button === THREE.MOUSE.LEFT) {
      ...
      if (intersects.length > 0) {
        ...
        if (mode === 'translate') {
          enabledTranslate &&
            translateStartPos.copy(
              ThreeUtils.getEventWorldPosition(event, domElement, camera)
            );
          ...
          if (enabledScale) {
            ...
            scalePivotLinePoint1 = ThreeUtils.getObjectWorldPosition(
              scaleGizmoGroup.getObjectByName(scalePivotLinePoint1Name)
            );
            scalePivotLinePoint2 = ThreeUtils.getObjectWorldPosition(
              scaleGizmoGroup.getObjectByName(scalePivotLinePoint2Name)
            );

            const scaleTouchPoint = ThreeUtils.getObjectWorldPosition(
              scaleGizmoGroup.getObjectByName(mode)
            );
            scaleStartDistance = computePointToLineDistance(
              scalePivotLinePoint1,
              scalePivotLinePoint2,
              scaleTouchPoint
            );
  ...
  function updateGizmo() {
    ...
    ThreeUtils.setObjectWorldPosition(
      gizmoGroup,
      ThreeUtils.getObjectWorldPosition(object)
    );
    ...
    ThreeUtils.setObjectWorldQuaternion(
      gizmoGroup,
      ThreeUtils.getObjectWorldQuaternion(object)
    );
    ...
    if (enabledScale) {
      ...
      const z = 0;
      scaleGizmoGroup
        .getObjectByName(name + 1)
        .position.set(width / 2 + offset, height / 2 + offset, z);
      scaleGizmoGroup
        .getObjectByName(name + 2)
        .position.set(-width / 2 - offset, height / 2 + offset, z);
      scaleGizmoGroup
        .getObjectByName(name + 3)
        .position.set(-width / 2 - offset, -height / 2 - offset, z);
      scaleGizmoGroup
        .getObjectByName(name + 4)
        .position.set(width / 2 + offset, -height / 2 - offset, z);

      scaleGizmoGroup
        .getObjectByName(name + 5)
        .position.set(width / 2 + offset, 0, z);
      scaleGizmoGroup
        .getObjectByName(name + 6)
        .position.set(0, height / 2 + offset, z);
      scaleGizmoGroup
        .getObjectByName(name + 7)
        .position.set(-width / 2 - offset, 0, z);
      scaleGizmoGroup
        .getObjectByName(name + 8)
        .position.set(0, -height / 2 - offset, z);
...

    {
      ...
      geometry.vertices.push(
        new THREE.Vector3(width / 2 + offset, height / 2 + offset, 0)
      );
      geometry.vertices.push(
        new THREE.Vector3(-width / 2 - offset, height / 2 + offset, 0)
      );
  ...
  function generateRotateGizmo() {
    ...
    }
    {
      ...
      var line = new THREE.Line(
        geometry,
        new THREE.LineBasicMaterial({ color: 0x28a7e1 })
      );
  ...
  function setMouseCursor(cursorMode) {
    ...
    if (!cursorMode) {
      ...
      if (cursorMode.indexOf('scale') !== -1) {
        ...
        switch (cursorMode) {
          case 'scale1':
            anchorRadian = -Math.PI / 4;
            break;
          case 'scale2':
            anchorRadian = Math.PI / 4;
            break;
          case 'scale3':
            anchorRadian = (Math.PI / 4) * 3;
            break;
          case 'scale4':
            anchorRadian = (Math.PI / 4) * 5;
            break;
          case 'scale5':
            anchorRadian = -Math.PI / 2;
            break;
          case 'scale6':
            anchorRadian = 0;
            break;
        ...
        const currentAnchorDirection = Math.round(
          currentAnchorRadian / (Math.PI / 4)
        );

        switch (currentAnchorDirection) {
          case 0:
            domElement.style.cursor = 'n-resize';
            break;
          case 1:
            domElement.style.cursor = 'nw-resize';
            break;
          case 2:
            domElement.style.cursor = 'w-resize';
            break;
          case 3:
            domElement.style.cursor = 'sw-resize';
            break;
          case 4:
          case -4:
            domElement.style.cursor = 's-resize';
            break;
          case -3:
            domElement.style.cursor = 'se-resize';
            break;
          case -2:
            domElement.style.cursor = 'e-resize';
            break;
          case -1:
            domElement.style.cursor = 'ne-resize';
            break;
        }
      } else if (cursorMode === 'translate') {
        domElement.style.cursor = 'all-scroll';
      } else if (cursorMode === 'rotate') {
        domElement.style.cursor = `url(${ico}), default`;
      }
    }
  }
  ...
  function handleMouseMoveTranslate(event) {
    ...
    translateEndPos.copy(
      ThreeUtils.getEventWorldPosition(event, domElement, camera)
    );
    translateDeltaPos.subVectors(translateEndPos, translateStartPos);
    const targetPos =
      ThreeUtils.getObjectWorldPosition(object).add(translateDeltaPos);
  ...
  function handleMouseMoveRotate(event) {
    ...
    const eventWorldPos = ThreeUtils.getEventWorldPosition(
      event,
      domElement,
      camera
    );
  ...
  function handleMouseMoveScale(event) {
    ...
    const distance = computePointToLineDistance(
      scalePivotLinePoint1,
      scalePivotLinePoint2,
      scaleEndPos
    );
    const targetDistance =
      scaleStartDistance + (distance - scaleStartDistance) * 2;
  ...
  function computePointToLineDistance(p1, p2, p) {
    ...
    const distance = Math.abs(
      (a * p.x + b * p.y + c) / Math.sqrt(a * a + b * b)
    );
    return distance;
  }
...
THREE.TransformControls2D.prototype = Object.assign(
