...
class TransformControls2D extends Object3D {
  ...
  initFramePeripherals() {
    ...
    this.framePeripheral = this.createPeripheral([['frame', frame]]);
  ...
  initTranslatePeripherals() {
    ...
    this.translatePeripheral = this.createPeripheral([['translate', plane]]);
  ...
  initScalePeripherals() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      definitions.push(['scale', circleOuter], ['scale', circleInner]);
    }
  ...
  updateMatrixWorld(force) {
    if (this.object) {
      ...
      const peripherals = [
        ...
        this.translatePeripheral,
        this.rotatePeripheral,
        this.scalePeripheral,
        this.scalePicker,
      ];
...

      {
        ...
        geometry.vertices.push(
          new Vector3(width / 2 + offset, height / 2 + offset, 0)
        );
        geometry.vertices.push(
          new Vector3(-width / 2 - offset, height / 2 + offset, 0)
        );
      ...
      // rotate
      const top = new Vector3(0, size.y / 2, 0.1).applyMatrix4(
      ...
      for (const handle of this.scalePeripheral.children.concat(
        this.scalePicker.children
      )) {
  ...
  updateMouseCursor() {
    switch (this.mode) {
      case 'translate':
        ...
        break;
      case 'rotate':
        document.body.style.cursor = `url(${ico}), default`;
        break;
      case 'scale': {
        ...
        const currentAnchorDirection =
          Math.round(currentAnchorRadian / (Math.PI / 4)) % 8;
  ...
  onMouseDown(coord) {
    ...
    const intersectPoint = new Vector3()
      ...
      .add(direction.multiplyScalar(scalar));
    ...
    switch (this.mode) {
      case 'scale': {
        ...
        this.scalePivotPoint
          .set((-size.x / 2) * xDirection, (-size.y / 2) * yDirection, 0)
          .applyMatrix4(this.object.matrixWorld);
        this.scaleCenterPoint
          .set(0, 0, 0)
          .applyMatrix4(this.object.matrixWorld);
        this.scaleMovingPoint
          .set((size.x / 2) * xDirection, (size.y / 2) * yDirection, 0)
          .applyMatrix4(this.object.matrixWorld);
  ...
  onMouseMove(coord) {
    ...
    switch (this.mode) {
      ...
      case 'rotate': {
        ...
        const clockPoint = new Vector3().subVectors(
          this.pointEnd,
          this.object.getWorldPosition()
        );
        const quaternion = ThreeUtils.getQuaternionBetweenVector3(
          clockPoint,
          up
        );
      ...
      case 'scale': {
        ...
        const direction2 = new Vector3()
          .subVectors(this.scaleMovingPoint, this.scaleCenterPoint)
          .normalize();
        const movement = new Vector3()
          .copy(direction2)
          .multiplyScalar(new Vector3().copy(direction2).dot(offset));

        const movingPoint = new Vector3()
          .copy(this.scaleMovingPoint)
          .add(movement);

        const l1 = new Vector3()
          .subVectors(this.scaleMovingPoint, this.scalePivotPoint)
          .length();
        const l2 = new Vector3()
          .subVectors(movingPoint, this.scalePivotPoint)
          .length();

        this.object.scale.copy(this.scaleStart).multiplyScalar(l2 / l1);
        this.object.position
          .copy(this.positionStart)
          .add(movement.multiplyScalar(0.5));

        // const diagonal = new Vector3()
        //   .copy(this.scaleMovingPoint)
        //   .sub(this.scalePivotPoint);
        ...
        // const rDiagonal = new Vector3()
        //   .copy(diagonal)
        //   .applyQuaternion(quaternionInverse);
        // const rNDiagonal = new Vector3()
        //   .copy(nDiagonal)
        //   .applyQuaternion(quaternionInverse);

        // const scale = new Vector3(
        //   Math.abs(isZero(rDiagonal.x) ? 1 : rNDiagonal.x / rDiagonal.x),
        //   Math.abs(isZero(rDiagonal.y) ? 1 : rNDiagonal.y / rDiagonal.y),
        //   1
        // );
        ...
        //   rNDiagonal.multiplyScalar(0.5),
        //   rDiagonal.multiplyScalar(0.5)
        // );
        // movement.x = isZero(rDiagonal.x) ? 0 : movement.x;
        // movement.y = isZero(rDiagonal.y) ? 0 : movement.y;

        // this.object.scale
        //   .copy(this.scaleStart)
        //   .multiplyVectors(this.scaleStart, scale);
        // this.object.position
        //   .copy(this.positionStart)
        //   .add(movement.applyQuaternion(quaternion));
