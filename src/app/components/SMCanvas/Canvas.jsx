...
class Canvas extends Component {
  ...
  getVisibleWidth() {
    return (
      this.node.current &&
      this.node.current.parentElement &&
      this.node.current.parentElement.clientWidth
    );
  }
  ...
  getVisibleHeight() {
    return (
      this.node.current &&
      this.node.current.parentElement &&
      this.node.current.parentElement.clientHeight
    );
  }
    ...
    const tween = new TWEEN.Tween(object)
      ...
      .onUpdate(() => {
        ...
        this.camera.position.x =
          this.controls.target.x + Math.sin(rotation) * dist;
        this.camera.position.y =
          this.controls.target.y - Math.cos(rotation) * dist;
  ...
  toTop() {
    ...
    const to = {
      rotationX:
        Math.round(this.camera.rotation.x / (Math.PI / 2)) * (Math.PI / 2) -
        Math.PI / 2,
    };
    const tween = new TWEEN.Tween(object)
      .to(to, ANIMATION_DURATION)
      .onUpdate(() => {
        ...
        this.camera.position.y =
          this.controls.target.y - Math.sin(rotation) * dist;
        this.camera.position.z =
          this.controls.target.z + Math.cos(rotation) * dist;
      });
    this.startTween(tween);
  }
  ...
  updateTransformControl2D() {
    this.transformSourceType === '2D' &&
      this.transformControls &&
      this.transformControls.updateGizmo();
  }
