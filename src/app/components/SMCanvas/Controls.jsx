...
class Controls extends EventEmitter {
  ...
  onClick = (event) => {
    ...
    const distance = Math.sqrt(
      (this.mouseDownPosition.x - mousePosition.x) ** 2 +
        (this.mouseDownPosition.y - mousePosition.y) ** 2
    );
    ...
    if (distance < 0.004 && this.selectableObjects) {
      ...
      const intersect = this.ray.intersectObjects(
  ...
  handleMouseMoveRotate = (event) => {
    ...
    this.rotate(
      event.clientX - this.rotateStart.x,
      event.clientY - this.rotateStart.y
    );
    this.rotateStart.set(event.clientX, event.clientY);
    this.updateCamera();
  };
  ...
  handleMouseMovePan = (event) => {
    this.pan(
      event.clientX - this.panPosition.x,
      event.clientY - this.panPosition.y
    );
