    ...
    if (controlPoints && controlPoints.length > 7) {
      ...
      if (controlPoints.length === 4) {
        ...
        for (let t = 0; t <= 1 + EPSILON; t += 0.02) {
          p0 =
            (1 - t) ** 3 * controlPoints[0].x +
            3 * t * (1 - t) ** 2 * controlPoints[1].x +
            3 * t ** 2 * (1 - t) * controlPoints[2].x +
            t ** 3 * controlPoints[3].x;
          p1 =
            (1 - t) ** 3 * controlPoints[0].y +
            3 * t * (1 - t) ** 2 * controlPoints[1].y +
            3 * t ** 2 * (1 - t) * controlPoints[2].y +
  ...
  for (const entities of dxf.entities) {
    ...
    if (
      ...
      const xrad = Math.sqrt(
        entities.majorAxisEndPoint.x ** 2 + entities.majorAxisEndPoint.y ** 2
      );
