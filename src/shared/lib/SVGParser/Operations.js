...
export function applyMatrix4(svg, m) {
  ...
  svg.viewBox[2] =
    Math.max(...corners.map((corner) => corner[0])) - svg.viewBox[0];
  svg.viewBox[3] =
    Math.max(...corners.map((corner) => corner[1])) - svg.viewBox[1];
...
export function rotate(svg, radian) {
  ...
  const anchorPoint = [
    svg.viewBox[0] + svg.viewBox[2] * 0.5,
    svg.viewBox[1] + svg.viewBox[3] * 0.5,
  ];

  const move1 = new Matrix4().makeTranslation(
    -anchorPoint[0],
    -anchorPoint[1],
    0
  );
  ...
  const move2 = new Matrix4().makeTranslation(
    anchorPoint[0],
    anchorPoint[1],
    0
  );
