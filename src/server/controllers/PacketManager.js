...
class PacketManager {
  ...
  packFeeder(content) {
    ...
    const dataLength =
      eventIDBuffer.length + indexBuffer.length + contentBuffer.length;
    const dataBuffer = Buffer.concat(
      [eventIDBuffer, indexBuffer, contentBuffer],
      dataLength
    );
    ...
    const buffer = Buffer.concat(
      [metaBuffer, dataBuffer],
      metaBuffer.length + dataBuffer.length
    );
    return buffer;
  }
  ...
  fileOperationRequestPrintFile(filename) {
    ...
    const contentBuffer = Buffer.concat(
      [operationBuffer, filenameBuffer],
      operationBuffer.length + filenameBuffer.length
    );
  ...
  statusSyncMachineStatus(x, y, z, e) {
    ...
    const contentBuffer = Buffer.concat(
      [subEventBuffer, posBuffer],
      subEventBuffer.length + posBuffer.length
    );
  ...
  changeCalibrationZOffset(offset) {
    ...
    const contentBuffer = Buffer.concat(
      [operationBuffer, offsetBuffer],
      operationBuffer.length + offsetBuffer.length
    );
  ...
  setLaserFocus(focusHeigh) {
    ...
    const contentBuffer = Buffer.concat(
      [operationBuffer, sizeBuffer],
      operationBuffer.length + sizeBuffer.length
    );
    return this.buildPacket(
      SETTINGS_REQUEST_EVENT_ID,
  ...
  enterSetFocus(laserState) {
    ...
    const sizeArray = toByte(
      [txtFocusX * 1000, txtFocusY * 1000, txtFocusZ * 1000],
      4
    );
    const sizeBuffer = Buffer.from(sizeArray, 'utf-8');
    const contentBuffer = Buffer.concat(
  ...
  laserMoveRequire(laserState) {
    const { txtMovementZ, txtMovementX, txtMovementY, relativeMode } =
      laserState;
    const operationID = new Uint8Array(1);
    if (relativeMode) {
      operationID[0] = 0x03;
    ...
    const operationBuffer = Buffer.from(operationID, 'utf-8');
    const sizeArray = toByte(
      [txtMovementX * 1000, txtMovementY * 1000, txtMovementZ * 1000],
      4
    );
    const sizeBuffer = Buffer.from(sizeArray, 'utf-8');
    const contentBuffer = Buffer.concat(
  ...
  setMachineSetting(machineSetting) {
    ...
    const offsetArray = toByte(
      [xOffset * 1000, yOffset * 1000, zOffset * 1000],
      4
    );
    const directionArray = toByte(
      [
        xHomeDirection,
        yHomeDirection,
        zHomeDirection,
  ...
  sendUpdatePacket(index) {
    ...
    const contentBuffer = Buffer.concat(
      [metaBuffer, packetBuffer],
      metaBuffer.length + packetBuffer.length
    );
  ...
  parseOriginUpdateFile(filename, type) {
    ...
    const date = new Date();
    const bVersion = Buffer.from(
      `Snapmaker_${date.getFullYear()}-${
        date.getMonth() + 1
