...
class MarlinController {
  ...
  query = {
    ...
    issue: () => {
      ...
      if (this.query.type === QUERY_TYPE_POSITION) {
        ...
        this.writeln('M105'); // 作废，改为M115 S5自动发送
        this.lastQueryTime = now;
  ...
  queryState = (() => {
    let index = 0;
    const typeOf3dp = [QUERY_TYPE_POSITION, QUERY_TYPE_ENCLOSURE];
    const typeOfLaser = [QUERY_TYPE_POSITION, QUERY_TYPE_ORIGIN_OFFSET];
    const type = [QUERY_TYPE_POSITION, QUERY_TYPE_ORIGIN_OFFSET];
    ...
    return () => {
      ...
      if (!this.query.type) {
        if (this.state.headType.substring(0, 3) === HEAD_TYPE_3DP) {
          this.query.type = typeOf3dp[index++ % typeOf3dp.length];
        } else if (this.state.headType.substring(0, 5) === HEAD_TYPE_LASER) {
          this.query.type = typeOfLaser[index++ % typeOf3dp.length];
        } else {
  ...
  constructor(options) {
    ...
    this.sender = new Sender(SP_TYPE_SEND_RESPONSE, {
      dataFilter: (line, context) => {
        if (line === WAIT) {
          ...
          log.debug(
            `Wait for the planner queue to empty: line=${
              sent + 1
    ...
    this.workflow.on('pause', () => {
      // this.emitAll('workflow:state', this.workflow.state);
      this.emitAll('workflow:state', { workflowState: this.workflow.state });
      if (this.sender.state.sent > 0) {
        this.sender.state.sent--;
        this.sender.state.received--;
      }
    });
    ...
    this.controller.on('originOffset', (res) => {
      log.silly(`controller.on('originOffset'): source=${
        this.history.writeSource
      },
                 line=${JSON.stringify(
                   this.history.writeLine
    ...
    this.controller.on('pos', (res) => {
      log.silly(
        `controller.on('pos'): source=${
          this.history.writeSource
        }, line=${JSON.stringify(this.history.writeLine)}, res=${JSON.stringify(
          res
        )}`
      );
      if (
    ...
    this.controller.on('temperature', (res) => {
      // do not connect, when serial port confilicted;
      const isSerialportConflicted =
        this.controller.state &&
        this.controller.state.workingStatus === 'WORKING_SCREEN';
      if (isSerialportConflicted) {
        this.emitAll('serialport:connected', {
          err: 'This port is being occupied',
        });
        return;
      }
      ...
      if (
        !this.controller.state ||
        this.controller.state.workingStatus === 'UNKONWN'
      ) {
        return;
      }
      if (!this.ready) {
        // if reconnect serial port by self, then call close() to reset state;
        const isSerialportBeeningUsedLastTime =
          this.controller.state &&
          this.controller.state.workingStatus === 'WORKING_PC';
        if (isSerialportBeeningUsedLastTime) {
          // /this.close();
          this.writeln('M2411 S3');
        }

        this.ready = true;
        this.emitAll('serialport:connected', { state: this.controller.state });
      }
      log.silly(`controller.on('temperature'): source=${
        this.history.writeSource
      },
                line=${JSON.stringify(
                  this.history.writeLine
    ...
    this.controller.on('selected', (res) => {
      log.silly(`controller.on('Selected origin'): source=${
        this.history.writeSource
      },
                 line=${JSON.stringify(
                   this.history.writeLine
    ...
    // this.controller.on('work:resume', () => {
    //     this.command(null, 'gcode:resume');
    // });

    this.controller.on('work:pause', () => {
      this.command(null, 'gcode:pause');
      // emit to frontend about work:pause
      this.emitAll('work:pause');
    });
    ...
    this.controller.on('ok', (res) => {
      ...
      if (this.workflow.state === WORKFLOW_STATE_RUNNING) {
        ...
        if (this.sender.state.hold) {
          ...
          if (received + 1 >= sent) {
            log.debug(
              `Continue sending G-code: sent=${sent}, received=${received}`
            );
            this.sender.unhold();
          }
        }
    ...
    this.controller.on('error', (res) => {
      ...
      if (this.workflow.state === WORKFLOW_STATE_RUNNING) {
        ...
        this.emitAll('serialport:read', {
          data: `> ${line.trim()} (line=${received + 1})`,
        });
        // this.emitAll('serialport:read', res.raw);
        this.emitAll('serialport:read', { data: res.raw });
    ...
    this.controller.on('serial:print:trigger:type:confirm', (res) => {
      if (!res) {
        log.error(
          `MarlinController serial:print:trigger:type:confirm get wrong res`
        );
        return;
      }
      // stage: "start" | "pause" | "resume" | "stop"
      // type: "PC" | "SC"
      const { stage, type } = res;
      log.debug(
        `MarlinController serial:print:trigger:type:confirm: stage: ${stage} type: ${type}`
      );
    });

    this.controller.on('serial:print:trigger:ok', (res) => {
      if (!res) {
        log.error(`MarlinController serial:print:trigger:ok get wrong res`);
        return;
      }
      ...
      if (stage === 'start') {
        ...
        this.workflow.pause();
        // waiting printer action finished
        this.emitAll('serialport:print:action:wait', { isWaiting: true });
    ...
    this.controller.on('serial:print:execute:ok', (res) => {
      if (!res) {
        log.error(`MarlinController serial:print:execute:ok get wrong res`);
        return;
      }
      ...
      // printer action finished
      this.emitAll('serialport:print:action:wait', { isWaiting: false });

      log.debug(`MarlinController serial:print:execute:ok: stage: ${stage}`);
    });

    this.controller.on('serial:print:error', (res) => {
      if (!res) {
        log.error(`MarlinController serial:print:error get wrong res`);
        return;
      }
      ...
      if (blockGcodeSend || shouldPause) {
        this.command(null, 'gcode:pause', ...sendCommands);
      }

      this.emitAll('serialport:print:error', {
        data: { stage, errCode, msg, showWarningModal, canUserClose },
      });
      log.debug(`Get MarlinController serial:print:error ${res}`);
    });

    this.controller.on('serial:print:door:closed', () => {
      this.emitAll('serialport:print:door:closed', {});
    });
    ...
    this.queryTimer = setInterval(() => {
      ...
      // }
      // this.queryState();  //fix auto upload
  ...
  open(callback = noop, connectionTimeout = this.options.connectionTimeout) {
    ...
    this.serialport = new SerialConnection({
      ...
      writeFilter: (data, context = {}) => {
        ...
        let { jogSpeed, workSpeed, headStatus, headPower } = {
          ...this.controller.state,
        };
        ...
        interpret(line, (cmd, params) => {
          ...
          if (cmd === 'M3') {
            ...
            if (params.P !== undefined) {
              ...
              headPower = ensureRange(params.S, 0, 100);
            }
          }
          if (cmd === 'M5') {
            headStatus = false;
    ...
    this.serialport.open((err) => {
      ...
      setTimeout(() => this.writeln('M2005'));
      setTimeout(() => this.writeln('M2051'), 100);
      setTimeout(() => this.writeln('M2007'), 150);
      setTimeout(() => this.writeln('M105'), 200);
      setTimeout(() => this.writeln('M2155 S4'), 200);
  ...
  command(socket, cmd, ...args) {
    const handler = {
      ...
      'gcode:start': () => {
        ...
        this.writeln('M2411 S2'); // IVI work start
      },
      'gcode:resume': () => {
        this.event.trigger('gcode:resume');

        this.writeln('M2411 S1'); // IVI work resume
      },
      'gcode:pause': () => {
        ...
        setTimeout(() => {
          this.writeln('M2410'); // IVI work pause
        }, 100);
      },
      'gcode:stop': () => {
        ...
        this.writeln('M2411 S3'); // IVI work stop
      },
      feedhold: () => {
      ...
      'switch hex mode': () => {
        this.controller.state.hexModeEnabled =
          !this.controller.state.hexModeEnabled;
      },
      'force switch': () => {
