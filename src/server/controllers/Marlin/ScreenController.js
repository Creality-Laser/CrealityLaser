...
class ScreenController {
        ...
        switch (typeof packetData) {
          ...
          case 'object':
            ...
            if (data[0] === 0x08 && data[1] === 0x01) {
              ...
              this.emitAll('machine:settings', {
                settings: this.controller.state.machineSetting,
              });
  ...
  constructor(options) {
    ...
    this.controller.on('temperature', (res) => {
      ...
      if (
        includes(
          [WRITE_SOURCE_CLIENT, WRITE_SOURCE_FEEDER, WRITE_SOURCE_SENDER],
          this.history.writeSource
        )
      ) {
        // this.emitAll('serialport:read', res.raw);
          ...
          switch (data) {
            ...
            case 'start print file\n':
              if (this.controller.gcodeFile) {
                ...
                outputData =
                  this.packetManager.statusRequestMachineStartPrint();
            ...
            case 'go to calibration point\n':
              outputData = this.packetManager.gotoCalibrationPoint(
                context.point
              );
              break;
            case 'change calibration z offset\n':
              outputData = this.packetManager.changeCalibrationZOffset(
                context.calibrationZOffset
              );
              break;
            case 'change calibration margin\n':
              outputData = this.packetManager.changeCalibrationMargin(
                context.calibrationMargin
              );
              break;
            /*
            ...
            case 'get setting\n':
              ...
              break;
            case 'set setting\n':
              outputData = this.packetManager.setMachineSetting(
                context.machineSetting
              );
              break;
            case 'set light mode\n':
            ...
            case 'set light status\n':
              outputData = this.packetManager.setLightStatus(
                context.lightStatus
              );
              break;
            ...
            case 'set laser focus\n':
              outputData = this.packetManager.setLaserFocus(
                context.focusHeight
              );
              break;
            ...
            case 'laser move require\n':
              outputData = this.packetManager.laserMoveRequire(
                context.laserState
              );
              break;
            ...
            case 'start update\n':
              if (this.controller.updateFile) {
                ...
                this.controller.state.updateCount =
                  this.packetManager.updateCount;
            ...
            case 'start update origin file\n':
              if (this.controller.updateFile) {
                this.packetManager.parseOriginUpdateFile(
                  this.controller.updateFile,
                  context.originFileUpdateType
                );
                this.controller.state.updateCount =
                  this.packetManager.updateCount;
                outputData = this.packetManager.startUpdate();
            ...
            default:
              if (source === WRITE_SOURCE_SENDER) {
                outputData = this.packetManager.packSender(
                  data,
