...
import { IP_WHITELIST, ERR_UNAUTHORIZED, ERR_FORBIDDEN } from './constants';
import DataStorage from './DataStorage';

const log = logger('app');

const renderPage =
  (view = 'index', cb = _.noop) =>
  (req, res) => {
...
const createApplication = () => {
  ...
  app.use((err, req, res, next) => {
    ...
    if (!bypass && err && err.name === 'UnauthorizedError') {
      ...
      log.warn(
        `ERR_UNAUTHORIZED: ipaddr=${ipaddr}, code="${
          err && err.code
  ...
  app.use(
    errnotfound({
      view: path.join('common', '404.html'),
      error: 'Not found',
    })
  );
  app.use(
    errserver({
      view: path.join('common', '500.html'),
