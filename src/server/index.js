...
const createServer = (options, callback) => {
  ...
  // configstore service
  log.info(
    `Loading configuration from ${chalk.yellow(JSON.stringify(profile))}`
  );
  config.load(profile);
  ...
  webappengine({ port, host, backlog, routes })
    .on('ready', (server) => {
      ...
      log.info(
        `Starting the server at ${chalk.cyan(
          `http://${realAddress}:${realPort}`
        )}`
      );
