...
function startServices(server) {
  ...
  socketServer.registerEvent(
    ...
    TaskManager.addGenerateToolPathTask
  );
  socketServer.registerEvent(
    'taskCommit:generateGcode',
    TaskManager.addGenerateGcodeTask
  );
...
function registerApis(app) {
  ...
  app.post(
    urljoin(settings.route, 'api/image/laserCaseImage'),
    api.image.laserCaseImage
  );
  app.post(urljoin(settings.route, 'api/image/process'), api.image.process);
  app.post(
    urljoin(settings.route, 'api/image/stock'),
    api.image.stockRemapProcess
  );
  app.post(urljoin(settings.route, 'api/image/trace'), api.image.processTrace);
  app.post(
    urljoin(settings.route, 'api/image/stitch'),
    api.image.processStitch
  );
  app.post(
    urljoin(settings.route, 'api/image/stitchEach'),
    api.image.processStitchEach
  );
  app.post(
    urljoin(settings.route, 'api/image/getPhoto'),
    api.image.processGetPhoto
  );
  app.post(
    urljoin(settings.route, 'api/image/takePhoto'),
    api.image.processTakePhoto
  );
  app.post(
    urljoin(settings.route, 'api/image/getCameraCalibration'),
    api.image.getCameraCalibrationApi
  );
  app.post(
    urljoin(settings.route, 'api/image/cameraCalibrationPhoto'),
    api.image.cameraCalibrationPhoto
  );
  app.post(
    urljoin(settings.route, 'api/image/setCameraCalibrationMatrix'),
    api.image.setCameraCalibrationMatrix
  );
  app.post(urljoin(settings.route, 'api/image/qrcode'), api.image.genQrcode);

  // Svg
  app.post(
    urljoin(settings.route, 'api/svg/convertRasterToSvg'),
    api.svg.convertRasterToSvg
  );
  app.post(
    urljoin(settings.route, 'api/svg/convertTextToSvg'),
    api.svg.convertTextToSvg
  );

  // ToolPath
  app.post(
    urljoin(settings.route, 'api/toolpath/generate'),
  ...
  // I18n
  app.get(
    urljoin(settings.route, 'api/i18n/acceptedLng'),
    api.i18n.getAcceptedLanguage
  );
  app.post(
    urljoin(settings.route, 'api/i18n/sendMissing/:lng/:ns'),
    api.i18n.saveMissing
  );

  // print3D
  app.post(urljoin(settings.route, 'api/file'), api.file.set);
  app.post(
    urljoin(settings.route, 'api/file/uploadCaseFile'),
    api.file.uploadCaseFile
  );
  app.post(
    urljoin(settings.route, 'api/file/uploadGcodeFile'),
    api.file.uploadGcodeFile
  );
  app.post(
    urljoin(settings.route, 'api/file/uploadUpdateFile'),
    api.file.uploadUpdateFile
  );
  ...
  app.post(
    urljoin(settings.route, 'api/file/recoverProjectFile'),
    api.file.recoverProjectFile
  );

  // app.get(
  //   urljoin(settings.route, 'api/printingQualityDefinitions/:series'),
  //   api.printingConfigs.getQualityDefinitions
  // );
  // app.get(
  //   urljoin(settings.route, 'api/printingMaterialDefinitions'),
  //   api.printingConfigs.getMaterialDefinitions
  // );
  // app.get(
  //   urljoin(settings.route, 'api/printingDefinition/:definitionId'),
  //   api.printingConfigs.getDefinition
  // );
  // app.get(
  //   urljoin(settings.route, 'api/printingRawDefinition/:definitionId'),
  //   api.printingConfigs.getRawDefinition
  // );
  // app.post(
  //   urljoin(settings.route, 'api/printingDefinition'),
  //   api.printingConfigs.createDefinition
  // );
  // app.delete(
  //   urljoin(settings.route, 'api/printingDefinition/:definitionId'),
  //   api.printingConfigs.removeDefinition
  // );
  // app.put(
  //   urljoin(settings.route, 'api/printingDefinition/:definitionId'),
  //   api.printingConfigs.updateDefinition
  // );
  // app.post(
  //   urljoin(settings.route, 'api/printingDefinition/upload'),
  //   api.printingConfigs.uploadDefinition
  // );
}

export { startServices, registerApis };
