...
function parse(line, initMeta, meta, options, startGcodeStart) {
  interpret(line, (cmd, params) => {
      ...
      if (!options.lastParams) {
        ...
        if (!initMeta[';estimated_time(s)']) {
          ...
          if (!params.F) {
            ...
            meta.estimatedTime +=
              (120 * distance) / (params.F + options.lastParams.F);
          }
        }
        options.lastParams = {
          ...options.lastParams,
    ...
    if (meta.headType === HEADTYPE_3DP) {
      ...
      meta.headType === HEADTYPE_LASER ||
      meta.headType === HEADTYPE_CNC
    ) {
      if (cmd === 'M3') {
        if (!initMeta[';power(%)']) {
...
function parseGcodeHeader(filename) {
  ...
  filter(initMeta);
  initMeta[';file_total_lines'] =
    meta.fileTotalLines - headerLength + Object.keys(initMeta).length + 2;
