...
export default class CncReliefToolPathGenerator extends EventEmitter {
  constructor(modelInfo, modelPath) {
    ...
    const maxDensity = Math.min(
      10,
      Math.floor(
        Math.sqrt(5000000 / transformation.width / transformation.height)
      )
    );
  ...
  generateToolPathObj() {
    let data = null;
    return Jimp.read(this.modelPath).then((img) => {
      if (this.invert) {
        img.invert();
      }
      ...
      this.targetWidth = Math.round(
        (this.targetWidth * img.bitmap.width) / width
      );
      this.targetHeight = Math.round(
        (this.targetHeight * img.bitmap.height) / height
      );
  ...
  parseImageToToolPathObj = (data) => {
    ...
    while (cutDown) {
      ...
      for (let i = 0; i < this.targetWidth; ++i) {
        ...
        for (let j = 0; j < this.targetHeight; ++j) {
          ...
          if (z > currentZ) {
            ...
            if (z < curDepth) {
              ...
              this.estimatedTime +=
                (this.getLineLength3D(startPoint, endPoint) * 60.0) /
                this.workSpeed;
              startPoint = { ...endPoint };
              cutDown = true;
        ...
        this.estimatedTime +=
          (this.getLineLength3D(startPoint, endPoint) * 60.0) / this.jogSpeed;
        startPoint = { ...endPoint };
        currentZ = this.safetyHeight;
        const p = i / (this.targetWidth - 1) / zSteps + cutDownTimes / zSteps;
        if (p - progress > 0.05) {
    ...
    return {
      ...
      mode,
      movementMode:
        headType === 'laser' && mode === 'greyscale'
          ? gcodeConfig.movementMode
          : '',
      data: this.toolPath,
  ...
  getLineLength3D(startPoint, endPoint) {
    if (
      (endPoint.X - startPoint.X < 1e-6 &&
        endPoint.Y - startPoint.Y < 1e-6 &&
        endPoint.Z - startPoint.Z < 1e-6) ||
