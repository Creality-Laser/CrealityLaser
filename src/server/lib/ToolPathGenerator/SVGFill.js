function mapPointToInteger(p, density) {
  return [Math.floor(p[0] * density), Math.floor(p[1] * density)];
}
...
function canvasToSegments(canvas, width, height, density) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (
      let x = isReverse ? width - 1 : 0;
      isReverse ? x >= 0 : x < width;
      x += sign
    ) {
      if (canvas[x][y] === 1) {
        const start = { x, y };
      ...
      if (shape.fill) {
        ...
        for (const path of shape.paths) {
          if (path.closed) {
            ...
            for (const point of path.points) {
              newPath.points.push(
                mapPointToInteger(point, options.fillDensity)
              );
            }
