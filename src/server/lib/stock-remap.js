...
function main(options) {
  ...
  return Jimp.read(imagePath).then(
    (originImage) =>
      new Promise((resolve) => {
        ...
        new Jimp(width, height, (err, image) => {
          ...
          for (let i = 0; i < width; ++i) {
            for (let j = 0; j < height; ++j) {
              ...
              image.bitmap.data[idx + 1] = Math.floor(
                mat[i][j].g / mat[i][j].cnt
              );
              image.bitmap.data[idx + 2] = Math.floor(
                mat[i][j].b / mat[i][j].cnt
              );
              image.bitmap.data[idx + 3] = Math.floor(
                mat[i][j].a / mat[i][j].cnt
              );
            }
          }
