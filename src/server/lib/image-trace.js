...
function getSVG(width, height, svgCollection) {
  return `<svg xmlns="http://www.w3.org/2000/svg"
        ...
        version="1.1">
        ${svgCollection.pathCollections.map((pathCollection) =>
          getPath(pathCollection)
        )}
      </svg>`;
}
...
function processSVG(svg) {
  ...
  for (const shape of svg.shapes) {
    if (shape.visibility) {
      ...
      fs.writeFileSync(
        `${DataStorage.tmpDir}/${filenames[outputCount++]}`,
        getSVG(width, height, svgCollection)
      );
    }
  }
  return {
...
function getMask(image, maskThreshold) {
  ...
  for (let i = 0; i < width; i++) {
    for (let j = 0; j < height; j++) {
      ...
      if (
        valueDiff >= maskThreshold ||
        greyscale.bitmap.data[index + 3] === 0
      ) {
...
async function trace(options) {
  ...
  if (path.extname(uploadName).toLowerCase() === '.svg') {
    ...
    const svg = await svgParser.parseFile(
